...
yesOrNo = function(txt) {
  ...
  if (_.contains(["yes", "yeah", "y", "ya", "yea", "yup", "yep", "true", "sure", "right", "certainly", "gladly", "of course", "absolutely", "cool", "totally", "got it", "ok"], txt)) {
    return true;
  } else if (_.contains(["no", "n", "nope", "false", "na", "not", "no way", "pass"], txt)) {
    return false;
  }
...
twilioValidatedCallback = function(callback) {
  var twilio = Npm.require('twilio');
  return function() {
    var valid = twilio.validateRequest(
      dotGet(Meteor, "settings.twilio.auth_token") || "1234",
      dotGet(this.request, "headers.x-twilio-signature"),
      Meteor.absoluteUrl(this.request.url.substring(1)),
      this.request.body);
    if (!valid) {
      this.response.writeHead(401, {"Content-Type": "text/plain"});
      return this.response.end("Authentication failed");
    }
