...
const neither = ['not sure'] // "not sure" would be interpreted as 'yes' without a special case
const yesPrimary = ['yes', 'yeah', 'y', 'ya', 'yea', 'yup', 'yep', 'true', 'sure', 'gladly', 'totally', 'totes', 'got it', 'ok', 'k', 'kk', 'umkay', 'sounds good', 'indeed', 'no problem', 'not a problem', 'okay', 'yess', 'yesss', 'yas', 'good', 'so far, so good', 'so far so good', 'of course']
const noPrimary = ['no', 'n', 'nope', 'false', 'na', 'not', 'pass', 'nah', 'nay', 'dont', 'do not', 'not cool', 'definitely not', 'def not', 'defs not', 'certainly not', 'course not', 'absolutely not', 'not ready', 'have not']
const yesSecondary = ['right', 'cool', 'definitely', 'def', 'defs', 'certainly', 'absolutely', 'ready']
const noSecondary = ['not']
...
yesOrNo = function (txt) {
  ...
  if (_.some(neither, containsWord) || txt.length > 15) { // response over 15 char is likely a question
    return null
  } else if (_.some(yesPrimary, containsWord)) {
    return true
...
// for brandedMessages.js college students interactions
replyOrNo = function (txt) {
  txt = txt.trim().toLowerCase().replace(/[^a-z ]/g, '')
  var regex = new RegExp('\\b' + 'reply' + '\\b')
  return regex.test(txt)
}
