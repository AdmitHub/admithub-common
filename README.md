...
Fields:
 ...
 - `facebook`: Type: Object. Optional. Contains information about the institution's facebook page, if our facebook app is subscribed to it. Subfields:
   - `pageId` Type: String. Required. The id facebook attaches to the relevant page; you can see this on the “About” part of the page (visible only if you have a role on the page). We use this to identify the messaging service of incoming facebook messages.
   - `pageAccessToken` Type: String. Required. A token generated by facebook for us to identify ourselves with, when we want to do something on behalf of a page, such as send messages.
   - `subscriptionMessagingStatus` Type: String. Default value: `'not_yet_requested'`. Indicates the status of the messaging service's request to facebook to send "subscription messaging" -- i.e., messages that are initiated more than 24 hours after the contact last messaged the bot. We check and update this value every time we send a campaign on a facebook messaging service. A value of `approved` indicates that the messaging service indeed has permission to send subscription messaging, last time we checked.
...
Contains information concerning points of contact at client institutions. Fields:
 ...
 - `associatedEmails` Type: \[Object\]. Optional. Each object contains information on a particular email association with the `collegeId` institution, including the kinds of topics that messages to that email address should be about. Subfields:
   ...
   - `region` Type: String. Optional. Region to match with a student Region for escalation
...
Contains information about specific states in the associated dialogs state-graph. A user moves through the dialog by traersing a path through the graph. These graphs will usually be trees, but need not be. Fields:
  ...
  - Any action can be performed as either an enter or exit action. The full list of actions are: `cancelCronJob`, `changeNumber`, `endSoftStop`, `save`, `sendEmail`, `scheduleJob` and `setPassiveOptOut`. `scheduleJob` can be used with either the `startDialog` or `sendMessage` methods.
...
A message recording information about campaigns scheduled to be initiated to student users. All outgoing campaigns have a corresponding `scheduledMessages` document. The front end uses this for display purposes, among other things. Fields:
  ...
  - `deliveryFailureUsers` Type: \[String\]. Required. Default value: `[]`. Elements of the array are ids of `brandedUserProfile` documents. If an id is in there, it means that at least one message failed to get delivered to the corresponding contact. We know this because either the channel reported this to us, or because there was something wrong when we inititally attempted to send a message -- data was malformed, or the messaging service didn't have the required permission to send to that contact. An id can be in only one of `deliveryFailureUsers`, `messagedUsers` and `optOutUsers` at a time.
  - `messagedUsers` Type: \[String\]. Required. Default value: `[]`. Elements of the array are ids of `brandedUserProfile` documents. If an id is in there, the corresponding contact has received at least one message in the scheduled campaign, and thus far no message have failed to be delivered (that we know about).  An id can be in only one of `deliveryFailureUsers`, `messagedUsers` and `optOutUsers` at a time.
  ...
  - `sendingUser` Type: String. Optional. User that is sending the campaign.
...
A record of each incoming and outgoing message. (To do: change name.) Fields:
  ...
  - `source` Type: String. Optional. Indicates the source of the message. Currently, there are exactly three logs such that their `source` value is not either `none specified` or `triage`. The other values are `mascot/conversation` and `conversation`. (To do: make a list of allowed values, and enforce. Possibly make required, depending on what is on that list.)
  - `subTransport` Type: String. Default value: ''. Some "transports" (more properly known as "channels") have more than one point of ingress. In particular, Facebook messages can come directly from Facebook messenger, or from a web plugin. This field distinguishes between these points of ingress, which you can think of as sub-transports.
