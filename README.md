...
Contains information about specific states in the associated dialogs state-graph. A user moves through the dialog by traersing a path through the graph. These graphs will usually be trees, but need not be. Fields:
  ...
  - Any action can be performed as either an enter or exit action. The full list of actions are: `cancelCronJob`, `changeNumber`, `endSoftStop`, `save`, `sendEmail`, `scheduleJob` and `setPassiveOptOut`. `scheduleJob` can be used to schedule a `startDialog` or `sendMessage` (intended for sending one off messages).
